...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    """
    with_prefix_mask = torch.tensor(batch.non_tensor_batch["prefix_uid"] != '0000-0000-0000-0000')
    ...
    # print(f"The response_length is: {response_length}, the max_response_length: {max_response_length}")
    ...
    if torch.any(with_prefix_mask):
        metrics.update(
            {
            # response length with prefix
            "response_length_prefix/mean": torch.mean(response_length[with_prefix_mask]).detach().item(),
            "response_length_prefix/max": torch.max(response_length[with_prefix_mask]).detach().item(),
            "response_length_prefix/min": torch.min(response_length[with_prefix_mask]).detach().item(),
            "response_length_prefix/clip_ratio": torch.mean(torch.eq(response_length[with_prefix_mask], max_response_length).float()).detach().item(),
            # reward
            "critic/score_prefix/mean": torch.mean(sequence_score[with_prefix_mask]).detach().item(),
            "critic/score_prefix/max": torch.max(sequence_score[with_prefix_mask]).detach().item(),
            "critic/score_prefix/min": torch.min(sequence_score[with_prefix_mask]).detach().item(),
            }
        )
    return metrics
