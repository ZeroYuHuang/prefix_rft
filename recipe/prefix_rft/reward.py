...
def get_custom_reward_fn(config, val=False):
...
def load_reward_manager(config, tokenizer, num_examine, val=False, **reward_kwargs):
    ...
    from recipe.prefix_rft_v2.reward_managers import RewardManager
    reward_manager_cls = RewardManager
    compute_score = get_custom_reward_fn(config, val=val)
...
def compute_reward(data: DataProto, reward_fn, cur_step):
    """
    ...
    """
    try:
        reward_result = reward_fn(data, return_dict=True, cur_step=cur_step)
        reward_tensor = reward_result["reward_tensor"]
        reward_extra_infos_dict = reward_result["reward_extra_info"]
    except Exception as e:
        print(f"Error in reward_fn: {e}")
        reward_tensor = reward_fn(data, cur_step, cur_step)
