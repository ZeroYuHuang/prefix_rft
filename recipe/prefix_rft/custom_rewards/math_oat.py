...
def rfn(solution_str: str, ground_truth: str, **kwargs):
    ...
    model_answer = extract_answer(kwargs['response_str'])
    gt_answer = ground_truth
    fast = kwargs.get("fast", False)
    
    if model_answer is None:
        return {"formatted": False, "score": 0.0, "acc": 0.0}
    ...
    if is_correct:
        formatted, reward = True, 1.0
    else:
        formatted, reward = True, 0.0
    return {
            "score": reward,
            "acc": reward,
            "formatted": formatted
...
def rfn_val(solution_str: str, ground_truth: str, **kwargs):
    return rfn(solution_str=solution_str, ground_truth=ground_truth, **kwargs)
...
def rfn_after_think_val(solution_str: str, ground_truth: str, **kwargs):
    generation = kwargs['response_str']
    generation = extract_final_answer_after_think(generation)
    if generation == "":
        generation = kwargs['response_str']
...
def rfn_after_think_formatted(solution_str: str, ground_truth: str, **kwargs):
    generation = kwargs['response_str']
    generation_after_think = extract_final_answer_after_think(generation)
    model_answer_after_think = extract_answer(generation_after_think)

    # 统一处理 ground truth 类型
    gt_answer = str(ground_truth) if isinstance(ground_truth, (float, int)) else ground_truth

    fast = kwargs.get("fast", False)
    
    if model_answer_after_think is None:
        # 尝试从完整回复中提取答案
        model_answer = extract_answer(generation)
        if model_answer is None:
            return {"score": 0.0, "acc": 0.0, "formatted": False}
        else:
            is_correct = False
            if isinstance(gt_answer, str):
                is_correct = grade(model_answer, gt_answer, fast)
                ...
                for gt in gt_answer:
                    if grade(model_answer, gt, fast):
                        ...
                        break
            if is_correct:
                return {"score": 0.9, "acc": 1.0, "formatted": False}
            else:
                return {"score": 0.0, "acc": 0.0, "formatted": False}
    else:
        model_answer = model_answer_after_think
        is_correct = False
        if isinstance(gt_answer, str):
            is_correct = grade(model_answer, gt_answer, fast)
        elif isinstance(gt_answer, list):
            for gt in gt_answer:
                if grade(model_answer, gt, fast):
                    ...
                    break
        if is_correct:
            return {"score": 1.0, "acc": 1.0, "formatted": True}
        else:
            return {"score": 0.0, "acc": 0.0, "formatted": True}

def rfn_after_think_formatted_val(solution_str: str, ground_truth: str, **kwargs):
    return rfn_after_think_val(solution_str=solution_str, ground_truth=ground_truth, **kwargs)
