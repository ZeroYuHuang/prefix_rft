...
# def is_latex_equal(str1, str2, executor, math_mode="legacy"):
...
# def is_equal(str1, str2, executor, math_mode="legacy"):
#     first_equal = is_equiv(str1, str2)
#     if first_equal:
#         return True
#     return is_latex_equal(str1, str2, executor, math_mode)
...
def extract_final_answer_after_think(response):
    ...
    # 使用 re.search 匹配并提取答案部分
    matches = re.search(pattern, response, re.DOTALL)
    if matches:
        return matches.group(1).strip()  # 提取捕获组内容，并去除可能的多余空格
    else:
        return ""

def extract_assistant_response(text: str):
    # gen_start = os.getenv("GENERATION_START", "<|im_start|>assistant")
    # gen_end = os.getenv("GENERATION_END", "<|endoftext|>")
    gen_start = os.getenv("GENERATION_START", None)
    gen_end = os.getenv("GENERATION_END", None)
    if gen_start is None:
        print("Please set GENERATION_START as the assistant template used, set to default")
        gen_start = "<|im_start|>assistant"
    if gen_end is None:
        print("Please set GENERATION_START as the assistant template used, set to default")
        gen_end = "<|endoftext|>"
...
def reward_fn_standard(solution_str, ground_truth, extract=0, **kwargs):
    """
    reward_fn_standard: 1.0 if the answer is right and 0 otherwise
    Could be utilized for validation
    """
    solution_str = extract_assistant_response(solution_str)
    if extract == 1:
        # extract the solution between <answer> and </answer>
        final_answer = extract_final_answer(solution_str)
    if extract == 2:
        # extract the solution after the end of thinking token
        final_answer = extract_final_answer_after_think(solution_str)
    else:
        final_answer = solution_str
    if final_answer == "":
        return 0.0
    iscorrect = is_equal(solution2answer(final_answer), solution2answer(ground_truth))
...
def reward_fn_format(solution_str, ground_truth, extract=False):
    ...
    if extract:
        ...
        final_answer = solution_str
    answer = solution2answer(final_answer)
    if answer == final_answer:  # the format is incorrect so we get the original final ans
        return -1.0
    iscorrect = is_equal(answer, solution2answer(ground_truth))
    score = 1.0 if iscorrect else 0.0
    return score
