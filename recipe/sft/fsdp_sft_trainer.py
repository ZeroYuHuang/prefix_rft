...
class FSDPSFTTrainer:
    ...
    def _build_dataloader(self, train_dataset, val_dataset):
        ...
        print(f"Number of examples in training dataset: {len(self.train_dataset)}")
        print(f"Number of examples in validation dataset: {len(self.train_dataset)}")
    ...
    def fit(self):
        ...
        # if it is set to -1, we save the checkpoint and test per epoch
        save_freq = self.config.trainer.save_freq
        if save_freq == -1:
            save_freq = self.steps_per_epoch
        test_freq = self.config.trainer.test_freq
        if test_freq == -1:
            test_freq = self.steps_per_epoch
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in tqdm(self.train_dataloader, total=self.steps_per_epoch, desc=f"Epoch {epoch + 1}/{self.config.trainer.total_epochs}", disable=rank != 0):
                ...
                is_valid_step = global_step % test_freq == 0
                is_save_step = global_step % save_freq == 0
